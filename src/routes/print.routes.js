...
// FONT_B en 58mm = ~42 caracteres por línea
const W = 42;
...
// Fecha manual 24h — sin locale para evitar "p. am" u otros artefactos
...
router.post('/sale', auth, function(req, res) {
  ...
  bufs.push([txt('Tel: 322 5251842  CC: 72289973')]);
  bufs.push([ln('=')]);

  // Titulo
  bufs.push([CMD.ALIGN_LEFT, CMD.BOLD_ON, txt('FACTURA DE VENTA'), CMD.BOLD_OFF]);
  ...
  bufs.push([CMD.BOLD_ON, txt('Gracias por su compra!'), CMD.BOLD_OFF]);
  bufs.push([txt('Conserve este recibo')]);
  bufs.push([ln('  ')]);
  bufs.push([txt('Baussas - 2026')]);
...
router.post('/repair', auth, function(req, res) {
  ...
  bufs.push([txt('Barranquilla, Colombia')]);
  bufs.push([txt('Calle 76f #22D-38')]);
  bufs.push([txt('Tel: 322 5251842  NIT: 72289973')]);
  bufs.push([ln('=')]);

  // Titulo
  bufs.push([CMD.ALIGN_LEFT, CMD.BOLD_ON, txt('ORDEN DE REPARACION'), CMD.BOLD_OFF]);
  ...
  if (r.priority === 'urgente') {
    bufs.push([ln('*'), CMD.ALIGN_CENTER]);
    bufs.push([CMD.BOLD_ON, txt('!! URGENTE !!'), CMD.BOLD_OFF]);
  ...
  bufs.push([CMD.BOLD_ON, txt('Gracias por confiar en nosotros!'), CMD.BOLD_OFF]);
  bufs.push([txt('  ')]);
  bufs.push([txt('Baussas - 2026')]);
  bufs.push([CMD.FEED3, CMD.CUT]);
